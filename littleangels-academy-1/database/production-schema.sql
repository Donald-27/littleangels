-- Little Angels Academy - Production Schema with Enterprise Security
-- Run this in Supabase SQL Editor for full functionality
...
-- JWT-based helper functions for enterprise multi-tenant security
...
-- Users table (extends Supabase auth.users with proper reference)
CREATE TABLE users (
...
-- Notifications table with proper foreign key name
...
-- Enterprise RLS Policies (JWT-based, no recursion)

-- Schools policies
CREATE POLICY "Users can view their school" ON schools
    FOR SELECT USING (id = app.current_school_id());
...
-- Users policies  
CREATE POLICY "Users can view their profile and school users" ON users
    FOR SELECT USING (
        id = auth.uid() OR 
...
CREATE POLICY "Admins can manage school users" ON users
...
CREATE POLICY "School users can view students" ON students
    FOR SELECT USING (
        school_id = app.current_school_id() AND (
            app.current_role() IN ('admin', 'teacher', 'accounts', 'driver') OR
            parent_id = auth.uid()
        )
    );
...
CREATE POLICY "Admins and teachers can manage students" ON students
    FOR ALL USING (
        school_id = app.current_school_id() AND 
        app.current_role() IN ('admin', 'teacher')
    )
    WITH CHECK (school_id = app.current_school_id());

-- Vehicles policies
CREATE POLICY "School users can view vehicles" ON vehicles
...
-- Routes policies
CREATE POLICY "School users can view routes" ON routes
    FOR SELECT USING (school_id = app.current_school_id());
...
CREATE POLICY "School users can view attendance" ON attendance
    FOR SELECT USING (
        school_id = app.current_school_id() AND (
            app.current_role() IN ('admin', 'teacher', 'driver', 'accounts') OR
            student_id IN (SELECT id FROM students WHERE parent_id = auth.uid())
        )
    );

CREATE POLICY "Drivers and admins can manage attendance" ON attendance
    FOR ALL USING (
        school_id = app.current_school_id() AND 
        app.current_role() IN ('admin', 'driver')
    )
    WITH CHECK (school_id = app.current_school_id());
...
CREATE POLICY "Users can view relevant payments" ON payments
    FOR SELECT USING (
        school_id = app.current_school_id() AND (
            ...
            app.current_role() IN ('admin', 'accounts')
        )
    );

CREATE POLICY "Accounts and admins can manage payments" ON payments
    FOR ALL USING (
        school_id = app.current_school_id() AND 
        app.current_role() IN ('admin', 'accounts')
    )
...
-- Trips policies
CREATE POLICY "School users can view trips" ON trips
    FOR SELECT USING (school_id = app.current_school_id());

CREATE POLICY "Drivers and admins can manage trips" ON trips
    FOR ALL USING (
        school_id = app.current_school_id() AND 
        app.current_role() IN ('admin', 'driver')
    )
...
CREATE POLICY "School users can view tracking" ON live_tracking
    FOR SELECT USING (school_id = app.current_school_id());

CREATE POLICY "Drivers and admins can update tracking" ON live_tracking
    FOR ALL USING (
        school_id = app.current_school_id() AND 
        app.current_role() IN ('admin', 'driver')
    )
...
CREATE POLICY "Users can view public settings" ON settings
    FOR SELECT USING (
        school_id = app.current_school_id() AND 
        (is_public = true OR app.is_admin())
    );
...
-- Add realtime support for enterprise features
ALTER PUBLICATION supabase_realtime ADD TABLE schools, users, students, vehicles, routes, attendance, live_tracking, notifications, payments, trips, settings;
